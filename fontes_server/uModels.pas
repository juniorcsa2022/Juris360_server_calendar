...
begin
  ...
  begin
    Result.AddPair('start_datetime', FormatDateTime('yyyy-mm-dd"T"hh:nn:ss"Z"', StartDateTime));
    Result.AddPair('end_datetime', FormatDateTime('yyyy-mm-dd"T"hh:nn:ss"Z"', EndDateTime));
  end;
  ...
  Result.AddPair('status', StatusToString(Status));
  Result.AddPair('created_at', FormatDateTime('yyyy-mm-dd"T"hh:nn:ss"Z"', CreatedAt));
  Result.AddPair('updated_at', FormatDateTime('yyyy-mm-dd"T"hh:nn:ss"Z"', UpdatedAt));
...
begin
  ...
  if Assigned(LValue) and (LValue is TJSONString) then
    StartDateTime := ISO8601ToDate(TJSONString(LValue).Value, True);

  LValue := AJSONObject.GetValue('end_datetime');
  if Assigned(LValue) and (LValue is TJSONString) then
    EndDateTime := ISO8601ToDate(TJSONString(LValue).Value, True);
